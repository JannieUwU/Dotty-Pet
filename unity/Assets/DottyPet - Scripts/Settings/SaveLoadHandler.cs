...
public class SaveLoadHandler : MonoBehaviour
{
    ...
    public static void ApplyAllSettingsToAllAvatars()
    {
        ...
        // Also apply scale to any custom VRM model loaded by LocalHttpServer.
        // That model has no AvatarAnimatorController, so it is not covered above.
        var httpServer = FindObjectOfType<LocalHttpServer>();
        if (httpServer != null)
        {
            var vrmModel = httpServer.GetCurrentVrmModel();
            if (vrmModel != null)
                vrmModel.transform.localScale = Vector3.one * data.avatarSize;
        }
    }
}
